...
module.exports = {
    ...
    register: async (req, res) =>{
        ...
        let regexEmail =  /(^[a-zA-Z0-9\.]{2,})+@([\w-]+\.)+[\w-]{1,3}$/;
        ...
        try {
            ...
            const token = jwt.sign({encod: user._id}, process.env.SECRETE_Key, {expiresIn: 60*60*24*1000});
            console.log(token)
    ...
    login: async (req,res)=>{
        ...
        try {
            ...
            console.log(u);
            const token = jwt.sign({encod: u._id}, process.env.SECRETE_Key, {expiresIn: 60*60*24*1000});
            console.log(token)
